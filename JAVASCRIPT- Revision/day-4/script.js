...
const addTodo = () => {//addtodo user se input lene ka kaam kr rha hai aur todolist me data ku push kr rha hai aur local storage pr set kr rha hai
    ...
    inputTodo.value = "";
    todoPriority.selectedIndex = 0;//jab display note call ho jaaye to jis jahag user se input liya gya hai.
...
const handlCompleteTodo =(i)=>{
    ...
    if(todoList[i].isCompleted){//agar iscompleted ki value true hogi to iske ander enter kregi 
        
        todoList[i].isCompleted.value = false;//condition true huyi to iscompleted ki value false assign ho jaayegi.
    }
    else{//if waali condition false huyi to iske ander enter hogi aur iscompleted ki value true assign ho jaayegi. 
        ...
        div.innerHTML = `<div class=" form-check m-2 d-flex align-items-center">
                        <div class=" shadow px-4 py-2 rounded border ${todo.priority == 'High' ? "border-danger" : (todo.priority == 'Medium') ? "border-warning" : "border-success"}">
                            <input onclick = "handlCompleteTodo(${i})" ${(todo.isCompleted) ? "checked" : ""} class="mb-1 ms-3 fs-1 " type="checkbox">
                                <span class=${(todo.isCompleted)?` fs-4 me-4 text-decoration-line-through` :`fs-4 me-4`}>${todo.title}</span> 
